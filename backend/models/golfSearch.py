...
def post_model_golfSearchList(search):
    print(f"models post_model_golfList start search : {search}")
    with engine.connect() as conn:
        #print("models post_model_golfList 쿼리 호출 전")
        # 1. 데이터를 DataFrame으로 불러옵니다 (이 부분은 동일).
        df = pd.read_sql(text(query_golfSearch), conn, params={"search": search})
